...
output_directory_path <- '/Users/christinechesebrough/Documents/MW_EEG_dir/MW_ratings/MW_ratings_processed_FreelyMoving_Ztrans'
...
processRatings <- function(file_path) {
  ...
  if ("Freely.Moving" %in% names(data)) {
    ...
    # Calculate the z-score for the 'Freely Moving' column
    data <- data %>%
      mutate(Freely.Moving = ifelse(is.na(Freely.Moving), NA_real_, (Freely.Moving - mean(Freely.Moving, na.rm = TRUE)) / sd(Freely.Moving, na.rm = TRUE)))
    
    
    # Transform the 'Freely Moving' column
    data$Freely.Moving <- case_when(
      data$Freely.Moving > 0 ~ "freely",
      data$Freely.Moving < 0 ~ "unmoving",
      TRUE ~ NA_character_
    )
  } else {
  ...
  new_file_name <- gsub('\\.csv', '_processed_freelymoving_z.csv', basename(file_path))
